...
public class RemoveUserRoleEndpoint : ICarterModule
{
    ...
    {
        app.MapDelete("users/{id}/roles/{roleName}", async (string id, string roleName, IMediator mediator, CancellationToken ct) =>
        {
        ...
        .WithDescription("Legacy compatibility endpoint. The single-role invariant prevents leaving a user without a role; assign a replacement role instead.")
